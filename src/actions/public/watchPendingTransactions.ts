...
} & (TTransportType extends 'webSocket'
  ?
      | {
          batch?: never
...
export function watchPendingTransactions<TTransportType extends string>(
  client: PublicClientArg<Transport<TTransportType>>,
  {
    batch = true,
    onError,
    onTransactions,
  ...
  }: WatchPendingTransactionsParameters<TTransportType>,
) {
  const enablePolling =
